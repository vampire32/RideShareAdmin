...
const WalletLogs = () => {
      const [anchorEl, setAnchorEl] = React.useState(null);
            ...
            const list = [];
						useEffect(() => {
							const databaseRef = firebase.database().ref("WalletLogs/");
							...
							{TotalUsers.map((item) => {
								return (
									<tr class="bg-white border-b " key={item.key}>
										...
										<td class="px-6 py-4">{item.userPhone}</td>
										<td class="px-6 py-4">{item.AccountNumber}</td>
										<td class="px-6 py-4">{item.AddAmount}</td>
										<td class="px-6 py-4">{item.CashOutAmount}</td>
